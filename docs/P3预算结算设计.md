...
| **R2** | 定标按 SKU 拆多供应商 vs **PRD 未定义多中标**（⚠️初版"PRD 单中标供应商"表述失真已订正：PRD 无单/多中标明文，全文单数语境 §6.6.4 L718；数据模型 L1201 `rfq.awarded_supplier_id` 单数字段，**结构上单供应商**）——**待用户业务决策** | 默认按 PRD 现状（单中标）；两方向影响面对照见 §0R.2，决策前 P2b 定标登记不动 `award` 约束 | `award/award_item` 约束、合同金额来源（连带审计 #14"份额校验"规则的存废） |
