...
| # | 问题 | 选项 | 默认建议 |
|---|---|---|---|
| Q1 | **无申请定标预算锚点**（CP-11） | A 定标提交即占（锚点=award）；B 合同生效时占；C 下单时占 | **A**（与"提交即占"口径一致，释放点=作废/驳回；**P2b-QA 第1轮补充定案**：①预算不足直接拦截提交，不走 BUDGET 升级审批——线下登记为人工行为，先调月度预算再登记，避免改 P3 已验证的 BudgetApprovalHandler；②占用生命周期必须闭环：驳回释放 + 重提先放后占幂等，QA P2b-3/4） |
