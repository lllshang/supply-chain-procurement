...
- **已定稿口径（Q1–Q4，不再待确认）**：① P2 只做标准链路端到端，日常/免比价与线下补录放 **P2b**（`purchase_apply.type` 预留，不动表结构）；② 采购申请**两级审批**（部门负责人→采购部）状态机，本阶段 `LocalApprovalGateway` 即审即过；③ 申请提交**预算软校验**（读 `budget_line` 余额、超限置 `budget_status=2` 仅提示），`used_amount` 不写；④ **最低价中标 + 异常价人工复核**；**R2 订正（PRD 留白 → 已裁决并落地）**：一询价单**单一中标供应商**（`award` 按 `inquiry` 唯一，`award_item` 明细行统一该供应商，不再按 SKU 拆多家；PRD 无单/多中标明文但数据模型 `rfq.awarded_supplier_id` 为单数 + 全文单数语境，commit `dfc62fc`）。
...
- **审批统一经 `ApprovalGateway`，P2 四个 bizType**：`PURCHASE_APPLY`（两级）/ `AWARD`（一级）/ `CONTRACT`（一级+金额阈值升级）/ `FULFILLMENT_ADJUST`（**R3 修订：一律审批**，取消原"阈值内免审"）。P3 阶段新增 `BUDGET`（预算升级/月度调整超20%）与 `SETTLEMENT`（结算审批，通过即核销预算）；R6 裁定付款登记免审批、无 `PAYMENT` bizType。回调映射到各状态机，幂等靠"任务状态一次性流转 + bizId 取最新任务"。
...
CREATE TABLE `award_item` (
  ...
  `supplier_id`        BIGINT        NOT NULL                COMMENT '中标供应商（R2 订正：单一中标，明细行统一该供应商）',
...
interface IAwardService extends IService<Award> {
    Long createAward(AwardSaveReqVO req);                  // 生成 award_no + award_item 明细（R2 订正：单一中标供应商，明细行统一该供应商）；重校验参与供应商准入
    ...
    void submit(Long id);                                  // 发起 ApprovalGateway(bizType=AWARD)；异常价（低于历史均价X%）标记人工复核提示
                                                           // R7 新增联动点（PRD BR-04 L1079/§6.4.3 L634）：提交时按 部门×科目×当月 调 P3 budgetOccupyService.checkOnly 再校验——
                                                           //   不足→拦截放行转 BUDGET 升级审批（通过后放行确认），remark 落"[预算升级]"；无申请来源/申请缺科目→显式跳过并留痕（QA2-02）
...
```
**状态机**（`OrderStatus`）：`CREATED →(首次到货) PARTIAL_RECEIVED →(足额) RECEIVED`；`CREATED/PARTIAL_RECEIVED →(取消) CANCELLED（释放额度）`。
**R5 修订（方向定案，落地排 P3b＝台账 D11）**：订单状态**收敛为不含** `SETTLED / PAID`——结算/付款是独立单据（settlement/payment）自身状态；订单维度的"已结算/已付清"为**派生展示字段**（settleProgress/paidProgress，由结算/付款单聚合计算），不进状态机、不落库流转。P3 当前 `SETTLED/PAID` 为过渡态，P3b 收敛时移除。
...
interface IFulfillmentAdjustService extends IService<FulfillmentAdjust> {
    ...
    void submit(Long id);                                  // R3 修订：一律进审批（ApprovalGateway, bizType=FULFILLMENT_ADJUST），取消 5% 免审 <!-- D5 -->
...
```
**状态机**（`AdjustStatus`，R3 修订）：`DRAFT →(提交) 审批中(1) → 生效(2) / 驳回(3)→DRAFT`；不再有免审直生效分支。可调整性校验（PRD L774）：仅"全部完成/已结算部分"的订单不可调整，**部分到货仍可调整**。
...
| `FULFILLMENT_ADJUST` | `adjust.submit()`（**R3：一律创建**） | `bizType=FULFILLMENT_ADJUST, bizId=adjustId, title="履约调整-{adjustNo}", payloadJson={adjustType,beforeAmount,afterAmount}` | `→生效(2)`（回写相关单据）；`→驳回(3)→DRAFT` | 一律审批（R3 修订，取消原"阈值内免审"） |
...
| **D8** 结算/付款/成本/报表 | `service_assess.deduct_amount` 为 P3 结算取数口子（§1.2.6）；**R5 修订：订单 `SETTLED/PAID` 废弃**（改 settleProgress/paidProgress 派生展示，见 §2.6 状态机修订） | **P3（已落地）** |
...
| # | 项 | 默认口径（本设计按此执行） | 风险 |
|---|---|---|---|
| Q5 | 订单变更/履约调整是否审批 | **`order_change` 免审直接生效（重跑三重校验）**；`fulfillment_adjust` **R3 修订：一律走 `FULFILLMENT_ADJUST` 审批**（PRD §6.7.3 L776"履约调整进入审批单据中心"无免审阈值；原"合同金额 5% 免审"口径作废，后端 `FulfillmentAdjustServiceImpl` 已落地） | 免审仅剩订单变更，靠留痕+对账兜底 |
