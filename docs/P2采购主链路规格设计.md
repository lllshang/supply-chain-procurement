...
- **AC**：① 仅 `APPLIED→APPROVED` 的申请可创建询价（`inquiry.apply_id` 关联）；② 发布前必须圈定 ≥1 家供应商；③ **发布时逐家调用 `GET /api/v1/suppliers/{id}/admission`，`qualified=false` 供应商剔除、不加入有效范围**（`invited=0`，`admission_snapshot` 记录剔除原因留痕；EXPIRED 资质/黑名单/停用均不合格；默认按 `supplier_sku` 绑定池**优选带入**，`bind_scope=0` 不强制）——混合 [合格,不合格] 时合格者正常加入，**剔除后有效范围为空才拒绝（4000「无可用供应商」）**（剔除式口径，主理人定稿 2026-09）；④ 供应商范围可增删，截标后锁定；⑤ 一个申请可多次询价（重新询价）。
...
- **字段与状态机**：`award`（已有 `inquiry_id/supplier_id/amount/status/remark`）**+ `award_no`（定标单号）、+ `apply_id`（追溯申请）**。**新建 `award_item`**：`award_id, sku_id, supplier_id, price, qty, qty_in_base_unit, conv_rate_snapshot, remark`（`order_item.source_award_item` 引用此表 id——P0 骨架已预留）。口径说明：`award_item.price` 与 `award.supplier_id` 分别为**基本单位口径单价**、**金额最大的主供应商**（单头列 NOT NULL，多供应商拆分时以明细为准）。
...
- **AC**：① 合同金额 = Σ**该供应商**来源定标明细金额（按 SKU 拆多供应商时各家按真实份额登记，非定标单总额，避免敞口翻倍；线下补录可手填）；② 附件走 `FileStorage`（`file_key`），支持多附件；③ 检索：按名称/供应商/状态/有效期范围组合查询，经数据权限过滤；④ 登记即校验供应商准入（`qualified=false` 拒绝）；⑤ 提交审批走 `ApprovalGateway`（bizType=`CONTRACT`），审批通过置 `EFFECTIVE` 并初始化 `available_amount=amount`。
