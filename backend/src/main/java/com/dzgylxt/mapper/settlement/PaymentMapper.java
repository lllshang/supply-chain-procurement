...
public interface PaymentMapper extends BaseMapper<Payment> {
    ...
    /** 结算单已付累计（仅 PAID；结算派生付款进度与订单付清判定口径。PB-01：PaymentStatus.PAID=1）。 */
    ...
    /** R5：订单已付累计（关联结算单的 PAID 付款；付清进度 paidProgress 口径。PB-01：PAID=1）。 */
    @Select("SELECT COALESCE(SUM(p.pay_amount), 0) FROM payment p"
            + " JOIN settlement s ON p.settlement_id = s.id"
            + " WHERE p.deleted = 0 AND s.deleted = 0 AND s.order_id = #{orderId} AND p.status = 1")
    BigDecimal sumPaidAmountByOrder(@Param("orderId") Long orderId);

    /**
     * 结算单付款承诺累计（QA #39：计入在途 + 已付——UNPAID 在途 + PAID 实付，
     * 防止多笔在途付款合计超出结算金额。PB-01：PaymentStatus 删除 PARTIAL/REJECTED，
     * 承诺 = 全部非删除付款 = status IN (0, 1)）。
     */
    @Select("SELECT COALESCE(SUM(pay_amount), 0) FROM payment"
            + " WHERE deleted = 0 AND settlement_id = #{settlementId} AND status IN (0, 1)")
